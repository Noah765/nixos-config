...
in {
  ...
  theme."zellij/config.kdl".text = theme: pkgs: ''
    ...
    ${(self.wrappers.multiplexer.apply {
      inherit pkgs;
      layoutDir = "${placeholder "out"}/${theme.name}/zellij/layouts";
  ...
  flake.wrappers = {
    ...
    multiplexer = {
      ...
      options.layoutDir = lib.mkOption {
        type = lib.types.path;
        default = "${placeholder config.outputName}/layouts";
        description = "Absolute path to the layouts directory.";
      };
