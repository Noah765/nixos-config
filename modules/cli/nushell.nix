{lib, ...}: {
  nixos = {
    ...
    config = lib.mkIf config.cli.nushell.enable {
      ...
      hm.programs.nushell = {
        ...
        environmentVariables =
          ...
          // rec {
            ...
            PROMPT_COMMAND = hmLib.nushell.mkNushellInline ''
              {
            ...
            '';
            TRANSIENT_PROMPT_COMMAND = hmLib.nushell.mkNushellInline "{ $env.PWD | str replace $nu.home-dir '~' | $in + '❯' }";
            PROMPT_COMMAND_RIGHT = hmLib.nushell.mkNushellInline "{ let duration = $env.CMD_DURATION_MS | into duration --unit 'ms'; if $duration > 5sec { $duration } }";
            TRANSITIVE_PROMPT_COMMAND_RIGHT = PROMPT_COMMAND_RIGHT;

            PROMPT_INDICATOR_VI_NORMAL = hmLib.nushell.mkNushellInline "{ if $env.LAST_EXIT_CODE == 0 { ':' } else { $'(ansi red):(ansi reset)' } }";
        ...
        settings = {
          ...
          float_precision = 3;
          hooks.display_output = hmLib.nushell.mkNushellInline ''
            {
              let value = $in
      ...
      hm.programs.carapace.enable = true;
    };
  };
}
