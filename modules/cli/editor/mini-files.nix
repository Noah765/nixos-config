{
  ...
  inherit (config.hm.lib.nixvim.utils) mkRaw;
  ...
  filterFunction = ''
    function(fs_entry)
      local is_in_excluded_entries = vim.list_contains(${toLua {} excludedEntries}, fs_entry.name)
      local ends_with_excluded_suffix = vim.list_contains(${toLua {} excludedSuffixes}, vim.iter(vim.gsplit(fs_entry.name, '.', { plain = true })):last())
      return MiniFiles.show_all or not is_in_excluded_entries and not ends_with_excluded_suffix
    end
  '';
in {
  options.cli.editor.mini-files.enable = lib.mkEnableOption "mini-files";

  config.cli.editor.settings = lib.mkIf config.cli.editor.mini-files.enable {
    # TODO Show help inside which-key
    plugins.mini.modules.files = {
      content.filter = mkRaw filterFunction;
      mappings.close = "<Esc>";
      windows = {
        preview = true;
        width_focus = 30;
