    ...
    let
      os =
        (modules.evalModules {
          ...
          ] ++ options.os.environment.persistence.type.nestedTypes.elemType.getSubModules;
        }).options;
    in
    #hm = options.hm.home.persistence.type.getSubOptions [ ];
    {
  ...
  config = mkIf cfg.enable {
    ...
    osModules = [
      ...
      (
        { ... }:
        {
          options.test = mkOption { type = types.anything; };
          config.test = test;
        }
      )
    ];
