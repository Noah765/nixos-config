{
  perSystem = {pkgs, ...}: {
    packages.install-os = pkgs.writeShellScriptBin "install-os" ''
      set -euo pipefail
      ...
      impermanence=$(nix eval ~/config#nixosConfigurations."$config".config.core.impermanence.enable --no-warn-dirty)

      if $impermanence; then
        disk=$(nix eval ~/config#nixosConfigurations."$config".config.core.impermanence.disk --no-warn-dirty --raw)
      ...
      sudo chown nixos:users /mnt/persist/system/etc/nixos -R
